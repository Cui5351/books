...
//      if(!referer||!(referer=='https://servicewechat.com/wxf5e611bcd30eb83d/17/page-frame.html'||referer=='https://www.mynameisczy.cn/'||referer=='https://mynameisczy.cn/'||referer=='https://servicewechat.com/wxf5e611bcd30eb83d/devtools/page-frame.html')){
...
function MountRouter(port,dbs,db_config,ws_config){
    ...
    app.ws('/user_chat',function(ws,req){
        ...
        app.post('/upload_avatar',(req,res)=>{
            ...
            let path=`https://www.mynameisczy.cn/user_avatar/${req.file.filename}`
            // 写入数据库
        ...
        app.post('/sign_in_day',(req,res)=>{
            ...
            const {openid}=data
            // 查看今天是否登录
            axios.post('https://www.mynameisczy.cn:5000/get_sign_in_day',data={openid:openid}).then(result=>{
...
app.post('/getOpenid',function(req,res){
    ...
    app.post('/goods_decrement',function(req,res){
        if(typeof req.body === 'string')
        req.body=JSON.parse(req.body)
        if(!req.body.hasOwnProperty('name')||!req.body.hasOwnProperty('openid')||!req.body.hasOwnProperty('goods')||!req.body.hasOwnProperty('person')){
            res.send({
                state:0,
                errorCode:1,
                errorMes:'缺少参数'
        ...
        const {name,openid,goods,person}=req.body
        // 查询用户的信息，并获取权限（）
        ...
        query(dbs,'books.user_info','user_privilege',{nickName:name,openid:openid}).then(v=>{
            ...
            // 查询到
                // 拿到权限
                let {user_privilege}=v[0]
                    // 有权限
                        // 根据权限去user_feedback里去寻找回复的参数
                        // author可以拿到全部的回复
                    if(user_privilege === '小说提供猿'||user_privilege === 'author'){ 
                        query(dbs,'books.user_info','shops',{nickName:person.nickName,openid:person.openid}).then(v2=>{
                        if(v2.length){
                            let {shops}=v2[0]
                            // 将其商品进行扣除
                            ...
                            for(let i=0;i<shops.length;i++){
                                if(shops[i].name==goods){
                                    if(shops[i].count2>0){
                                        ...
                                        flag++
                                        // 商品成功兑换
                                        update(dbs,'books.user_info',{nickName:person.nickName,openid:person.openid},'shops',JSON.stringify(shops),'string')
                                        res.send({
                                            state:1,
                                            data:shops,
                                            errorCode:0
