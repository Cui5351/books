    ...
    function MountRouter(port,dbs,db_config,ws_config){
        ...
        app.ws('/user_chat',function(ws,req){
            ...
            app.post('/getAnswer',function(req,res){
                ...
                const {openid}=req.body
                // 查看openid
                query(dbs,'mynameisczy_asia.user_info',['author_answer','data_provide_answer'],{openid:openid}).then(e=>{
                    res.send({
                        state:1,
                        error:0,
                        value:e
    ...
    app.post('/login_user',function(req,res){
        ...
        query(dbs,'mynameisczy_asia.user_info',['avatarUrl','gender','nickName','score','author_answer','data_provide_answer','introduction'],{openid:openid}).then(e=>{
            // 存在
            // 将查询到的数据返回
            if(e.length){
                res.send({
    ...
    app.post('/getOpenid',function(req,res){
        ...
        //   }            
            ...
            app.post('/user_data_auto_update',(req,res)=>{
                ...
                if(typeof req.body == 'string')
                    req.body=JSON.parse(req.body)
                let {openid}=req.body
                query(dbs,'mynameisczy_asia.user_info',['avatarUrl','gender','nickName','score','author_answer','data_provide_answer'],{openid:openid}).then(e=>{
                    // 存在
                    // 将查询到的数据返回
                    if(e.length){
                        res.send({
                ...
                // 查询到
                    ...
                    const {user_privilege}=v[0]
                        ...
                        if(user_privilege === 'author'||user_privilege === '小说提供猿'){
                            query(dbs,'mynameisczy_asia.user_info',['nickName','avatarUrl','shops','openid']).then(value=>{
                                res.send({
                                    state:1,
                                    data:value,
                                    errorCode:0
        ...
        app.post('/goods_decrement',function(req,res){
            ...
            // 1：无权限
            // 2：登录失败，请返回登录页面
            query(dbs,'mynameisczy_asia.user_info',['user_privilege','shops'],{nickName:name,openid:openid}).then(v=>{
                if(v.length){
                // 查询到
                    // 拿到权限
                            ...
                            for(let i=0;i<shops.length;i++){
                                if(shops[i].name==goods){
                                    if(shops[i].count2>0){
                                        ...
                                        flag++
                                        // 商品成功兑换
                                        update(dbs,'mynameisczy_asia.user_info',{nickName:name,openid:openid},'shops',JSON.stringify(shops),'string')
                                        res.send({
                                            state:1,
                                            data:shops,
                                            errorCode:0
